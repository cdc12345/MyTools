...
public class DataGen {
	...
	public static void main(String[] args) {
		...
		pluginFactory.createVariable(VariableTypes.ATOMIC_NUMBER).setColor(254).setNullable(true)
				.setIgnoredByCoverage(true).buildAndOutput();
		...
		var en = pluginFactory.createDefaultLanguage()
				.appendLocalization("elementgui.enchantment.damage_bonus", "Damage Bonus");
		var cn = pluginFactory.createLanguage(Locale.CHINA)
				.appendLocalization("elementgui.enchantment.damage_bonus", "伤害加成");

		createProcedures(pluginFactory);
		createTriggers(pluginFactory);
		pluginFactory.createTrigger().setName("entity_pre_hurt_reload").appendDependency("x", BuiltInTypes.Number)
				.appendDependency("y", BuiltInTypes.Number).appendDependency("z", BuiltInTypes.Number)
				.appendDependency("world", BuiltInTypes.World).appendDependency("amount", VariableTypes.ATOMIC_NUMBER)
				.appendDependency("entity", BuiltInTypes.Entity)
				.appendDependency("damagesource", BuiltInTypes.DamageSource)
				.appendDependency("sourceentity", BuiltInTypes.Entity).initGenerator()
				.setLanguage(en, "TEntity Pre Hurt").setLanguage(cn, "T实体受伤前").setCancelable(true)
				.buildAndOutput();
		pluginFactory.createTrigger().setName("living_pre_heal").appendDependency("x", BuiltInTypes.Number)
				.appendDependency("y", BuiltInTypes.Number).appendDependency("world", BuiltInTypes.World)
				.appendDependency("amount", VariableTypes.ATOMIC_NUMBER).appendDependency("entity", BuiltInTypes.Entity)
				.setLanguage(en, "Living Pre Heal").setCancelable(true).buildAndOutput();
		pluginFactory.createTrigger().setName("fov_update").appendDependency("x", BuiltInTypes.Number)
				.appendDependency("y", BuiltInTypes.Number).appendDependency("z", BuiltInTypes.Number)
				.appendDependency("world", BuiltInTypes.World).appendDependency("amount", VariableTypes.ATOMIC_NUMBER)
				.appendDependency("entity", BuiltInTypes.Entity).initGenerator().setLanguage(en, "Fov Update")
				.setSide(Side.Client).buildAndOutput();
		pluginFactory.createTrigger("creativetab_content").appendDependency("tab", BuiltInTypes.String).initGenerator()
				.setLanguage(en, "BuildCreativeTab").buildAndOutput();

		//CreativeTab
		pluginFactory.createProcedure().setName("creativetab").markType().setColor(Color.BLUE)
				.setLanguage(en, "CreativeTab").initGenerator().buildAndOutput();
		pluginFactory.createProcedure("creativetab_inserbefore").setPreviousStatement(null).setNextStatement(null)
		...
		pluginFactory.createProcedure("creativetab_insertafter").setPreviousStatement(null).setNextStatement(null)
		...
		pluginFactory.createProcedure("atomic_number_set").appendArgs0InputValue("key", VariableTypes.ATOMIC_NUMBER)
				...
				.appendConstantNumber(0).buildAndReturn().setInputsInline(true).setNextStatement(null)
				.setPreviousStatement(null).setColor(BuiltInBlocklyColor.MATH.toString())
				.setToolBoxId(BuiltInToolBoxId.Procedure.MATH).setLanguage(en, "set %1 to %2").initGenerator()
				.buildAndOutput();
		pluginFactory.createProcedure("atomic_number_get").setColor(BuiltInBlocklyColor.MATH.toString())
				.appendArgs0InputValue("key", VariableTypes.ATOMIC_NUMBER).setLanguage(en, "get %1")
				.setOutput(BuiltInTypes.Number).setToolBoxId(BuiltInToolBoxId.Procedure.MATH).setGroup("name")
				.setInputsInline(true).initGenerator().buildAndOutput();

		//Debug
		pluginFactory.createProcedure("number_to_double").setColor(BuiltInBlocklyColor.MATH.toString())
				.appendArgs0InputValue("value", BuiltInTypes.Number).setOutput(BuiltInTypes.Number).setGroup("name")
				.setCategory(BuiltInToolBoxId.Procedure.ADVANCED).toolBoxInitBuilder().setName("value")
				.appendConstantNumber(0).buildAndReturn().setLanguage(en, "Double(%1)").initGenerator()
				.buildAndOutput();
		...
		pluginFactory.getToolKit().createOutputProcedure("classname_object",BuiltInTypes.String).setCategory(BuiltInToolBoxId.Procedure.ADVANCED)
				.setColor(BuiltInBlocklyColor.TEXTS.toString()).appendArgs0InputValue("value", (String) null)
                .setLanguage(en,"class name of %1").initGenerator().buildAndOutput();
