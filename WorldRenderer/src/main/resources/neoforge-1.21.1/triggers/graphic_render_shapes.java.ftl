...
public class ${name}Procedure {
  ...
  private static boolean begin(VertexFormat.Mode mode, VertexFormat format, boolean update) {
    if (${name}Procedure.bufferBuilder == null) {
      if (update)
        clear();
      if (${name}Procedure.vertexBuffer == null) {
        if (format == DefaultVertexFormat.POSITION_COLOR) {
          ...
          ${name}Procedure.bufferBuilder = Tesselator.getInstance().begin(mode, DefaultVertexFormat.POSITION_COLOR);
          return true;
        } else if (format == DefaultVertexFormat.POSITION_TEX_COLOR) {
          ${name}Procedure.mode = mode;
          ${name}Procedure.format = format;
          ${name}Procedure.bufferBuilder = Tesselator.getInstance().begin(mode, DefaultVertexFormat.POSITION_TEX_COLOR);
          return true;
        }
      }
    }
