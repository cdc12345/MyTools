    ...
    int length = Minecraft.useFancyGraphics() ? 10 : 5;
      ...
      if (range > 0) {
        ...
        alpha = alpha < 0 ? 0.0F : alpha;
			...
			bufferBuilder.addVertex((float) (x - camX - vecX + 0.5D), (float) (top - camY), (float) (z - camZ - vecZ + 0.5D)).setUv(0.0F, btm * 0.25F + anime).setColor(red, green, blue, alpha).setLight(packedLight);
			bufferBuilder.addVertex((float) (x - camX - vecX + 0.5D), (float) (btm - camY), (float) (z - camZ - vecZ + 0.5D)).setUv(0.0F, top * 0.25F + anime).setColor(red, green, blue, alpha).setLight(packedLight);
			bufferBuilder.addVertex((float) (x - camX + vecX + 0.5D), (float) (btm - camY), (float) (z - camZ + vecZ + 0.5D)).setUv(1.0F, top * 0.25F + anime).setColor(red, green, blue, alpha).setLight(packedLight);
			bufferBuilder.addVertex((float) (x - camX + vecX + 0.5D), (float) (top - camY), (float) (z - camZ + vecZ + 0.5D)).setUv(1.0F, btm * 0.25F + anime).setColor(red, green, blue, alpha).setLight(packedLight);
    ...
    int length = Minecraft.useFancyGraphics() ? 10 : 5;
      ...
      if (range > 0) {
        ...
        alpha = alpha < 0 ? 0.0F : alpha;
			...
			RenderSystem.setShader(GameRenderer::getParticleShader);
			BufferBuilder bufferBuilder = Tesselator.getInstance().begin(VertexFormat.Mode.QUADS, DefaultVertexFormat.PARTICLE);
			bufferBuilder.addVertex((float) (x - camX - vecX + 0.5D), (float) (top - camY), (float) (z - camZ - vecZ + 0.5D)).setUv(0.0F + du, btm * 0.25F + anime + dv).setColor(red, green, blue, alpha).setUv2(sky, block);
      bufferBuilder.addVertex((float) (x - camX - vecX + 0.5D), (float) (btm - camY), (float) (z - camZ - vecZ + 0.5D)).setUv(0.0F + du, top * 0.25F + anime + dv).setColor(red, green, blue, alpha).setUv2(sky, block);
      bufferBuilder.addVertex((float) (x - camX + vecX + 0.5D), (float) (btm - camY), (float) (z - camZ + vecZ + 0.5D)).setUv(1.0F + du, top * 0.25F + anime + dv).setColor(red, green, blue, alpha).setUv2(sky, block);
			bufferBuilder.addVertex((float) (x - camX + vecX + 0.5D), (float) (top - camY), (float) (z - camZ + vecZ + 0.5D)).setUv(1.0F + du, btm * 0.25F + anime + dv).setColor(red, green, blue, alpha).setUv2(sky, block);
			BufferUploader.drawWithShader(bufferBuilder.buildOrThrow());
		}
	}
      ...
      for (int i = -range; i <= range; ++i) {
        for (int j = -range; j <= range; ++j) {
          ...
          if (biome.hasPrecipitation()) {
              ...
              RenderSystem.setShaderTexture(0, ResourceLocation.withDefaultNamespace("textures/environment/rain.png"));
              renderRain(speed, ix + i, iz + j, range, color, constant);
            }
            if (snow && precipitation == Biome.Precipitation.SNOW) {
              RenderSystem.setShaderTexture(0, ResourceLocation.withDefaultNamespace("textures/environment/snow.png"));
